/* ══════════════════════════════════════════════════════════════════════════
   EL PARSER DEL PASE — una sola copia

   Convierte el texto plano de un documento del Drive en una lista de
   pacientes con sus secciones separadas.

   POR QUÉ ESTE ARCHIVO EXISTE
   ---------------------------
   Hasta el 2/9/2026 este código estaba DUPLICADO en api/parse-pase.js y
   api/sync-pases.js. Cada arreglo había que hacerlo dos veces, y cuatro veces
   pasó que se hiciera en uno solo. Para cuando se unificó, las dos copias ya
   habían divergido de verdad: la expresión que reconoce el encabezado
   "REQUERIMIENTOS / INTERCURRENCIAS" aceptaba espacios en una copia y no en
   la otra, así que según qué endpoint procesara el documento esa sección
   entera se reconocía o se perdía sin que nadie se enterara.

   Los dos endpoints importan de acá. Si hay que cambiar cómo se lee el pase,
   se cambia en este archivo y listo.
   ══════════════════════════════════════════════════════════════════════════ */

const FIELDS = [
  { key: "ap", re: /^AP\s*[:\-]/i, label: "Antecedentes" },
  { key: "req",
    // Se acepta la barra con o sin espacios, y también "E" o "Y" en lugar de
    // la barra: el encabezado se escribe a mano y aparece de las dos formas.
    // Las dos copias del parser tenían versiones distintas de esta expresión
    // —una aceptaba "REQUERIMIENTOS / INTERCURRENCIAS" y la otra no— así que
    // según qué endpoint procesara el documento, esa sección entera se
    // reconocía o se perdía.
    re: /^(REQUERIMIENTOS?\s*(?:\/|E|Y)?\s*INTERCURRENCIAS?|INTERCURRENCIAS?\s*(?:\/|E|Y)?\s*REQUERIMIENTOS?|REQ\s*\/\s*INTERCURRENCIAS?)/i,
    label: "Requerimientos / Intercurrencias" },
...
/* El último renglón de Requerimientos: es el estado actual del paciente, lo
   que la app muestra debajo del motivo de ingreso. También estaba duplicado. */
